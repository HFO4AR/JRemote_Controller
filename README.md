...
- **多种连接模式**：
  - **蓝牙模式（BLE）**：传统低功耗蓝牙连接
  - **Wi-Fi AP 模式**：手机连接 ESP32 热点，UDP 通信
  - **Wi-Fi 局域网模式**：手机与 ESP32 连接同一 Wi-Fi 网络
- **底部导航栏**：连接界面使用底部导航栏切换模式，支持左右滑动切换
- **WiFi 配网功能**：通过 BLE 配置 ESP32 的 WiFi SSID 和密码，支持开放网络
- **一键获取当前 WiFi**：自动读取手机当前连接的 WiFi 名称
- **实时状态反馈**：显示连接状态、信号强度和通信延迟
- **调试面板**：显示详细的通信日志和设备响应（MUC 消息以绿色显示）
...
1. 打开 `sample/esp32_udp_receiver/` 目录下的示例代码
2. 将代码上传到您的 ESP32 设备
3. 确保 ESP32 设备已启动并运行
...
1. 使用示例代码作为基础
   - `sample/esp32_udp_receiver/` - Wi-Fi UDP 接收端（支持 AP/Station 模式 + BLE 配网）
