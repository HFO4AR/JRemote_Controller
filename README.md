...
| 组件 | 角色 | 说明 |
|------|------|------|
| 手机 (App) | 控制器 | 发送控制指令（摇杆、按钮） |
| ESP32 | 接收机/网关 | 转发 App 指令到 MCU，回传 MCU 数据 |
| MCU | 被控端 | 接收指令控制执行机构，上传传感器数据 |
...
| 帧头 | 名称 | 摇杆精度 | 按钮数量 | 总长度 |
|------|------|----------|----------|--------|
| 0xA0 | 最简帧 | -127 ~ 127 | 8 个 | 6 字节 |
| 0xAA | 标准帧 | -127 ~ 127 | 32 个 | 9 字节 |
| 0xAC | 16位帧 | -32767 ~ 32767 | 32 个 | 17 字节 |
| 0xEE | 急停 | - | - | 1 字节 |

**多节点帧**：通过帧头中的路由字节区分目标节点，支持广播（0x00）或指定节点（0x01-0x06）。

**UART 帧**：ESP32 与 MCU 通信使用独立的帧头（0xC0-0xCB），支持可选 XOR 校验。

详细数据格式见 [JBus 协议文档](./docs/JBus_Protocol.md)

### 设备响应格式

设备响应（包括 Ping 响应、传感器数据、调试信息等）详见 [JBus 协议文档](./docs/JBus_Protocol.md)
...
- **帧格式**: 选择控制帧格式（最简6字节/标准9字节/16位17字节）
...
| 帧头 | 名称 | 数据长度 | 按钮数量 |
|------|------|----------|----------|
| 0xA0 | 控制数据（最简） | 6 字节 | 8 个 |
| 0xAA | 控制数据（标准） | 9 字节 | 32 个 |
| 0xAC | 控制数据（16位） | 17 字节 | 32 个 |
| 0xB0 | 控制数据（多节点最简） | 10 字节 | 8 个 |
| 0xB2 | 控制数据（多节点标准） | 11 字节 | 32 个 |
| 0xB3 | 控制数据（多节点16位） | 19 字节 | 32 个 |
| 0xBB | 用户数据 | 变长 | - |
| 0xB1 | 用户数据（多节点） | 变长 | - |
| 0xEE | 急停 | 1 字节 | - |
...
| 帧头 | 名称 | 数据长度 | 说明 |
|------|------|----------|------|
| 0xC0 | UART 控制（最简） | 6 字节 | 无校验 |
| 0xC1 | UART 控制（标准） | 9 字节 | 无校验 |
| 0xC2 | UART 控制（16位） | 17 字节 | 无校验 |
| 0xC3 | UART 用户数据 | 变长 | 无校验 |
| 0xC4 | UART 急停 | 1 字节 | 无校验 |
| 0xC8 | UART 最简+校验 | 7 字节 | XOR 校验 |
| 0xC9 | UART 标准+校验 | 10 字节 | XOR 校验 |
| 0xCA | UART 16位+校验 | 18 字节 | XOR 校验 |
| 0xCB | UART 用户数据+校验 | 变长 | XOR 校验 |
